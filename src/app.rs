...
pub fn run(cfg: Config, existing_content: Option<String>) -> Result<()> {
    ...
    let translator_status_init = match translator_cfg {
        Some(tcfg) => {
            ...
            // Fanout: any transcript line is mirrored to the UI immediately AND
            // forwarded to the translator. This means the UI works even if the
            // translator is still loading or has failed.
